...
import { IndiceWS } from '../../model/indice-ws';
...
export class TabelaIndiceComponent {
  ...
  clickedRows = new Set<IndiceWS>();
  selection = new SelectionModel<IndiceWS>(true, []);

  // Armazena todas as requisições que foram alteradas em tela
  private requisicoesAlteradas: IndiceWS[] = [];

  // Armazena o backup de todas as requisições alteradas
  private requisicoesBackup: IndiceWS[] = [];

  constructor(private _liveAnnouncer: LiveAnnouncer) {
    this.headerColumns = Colunas.colunasIndice;
