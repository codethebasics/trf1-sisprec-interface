...
import { CondicaoServidorTipoWS } from '../../model/condicao-servidor-tipo-ws';
...
export class TabelaServidorCondicaoTipoComponent {
  ...
  clickedRows = new Set<CondicaoServidorTipoWS>();
  selection = new SelectionModel<CondicaoServidorTipoWS>(true, []);

  // Armazena todas as requisições que foram alteradas em tela
  private requisicoesAlteradas: CondicaoServidorTipoWS[] = [];

  // Armazena o backup de todas as requisições alteradas
  private requisicoesBackup: CondicaoServidorTipoWS[] = [];

  constructor(private _liveAnnouncer: LiveAnnouncer) {
    this.headerColumns = Colunas.colunasServidorCondicaoTipo;
