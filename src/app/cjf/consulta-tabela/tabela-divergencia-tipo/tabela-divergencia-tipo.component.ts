...
import { DivergenciaTipoWS } from '../../model/divergencia-tipo-ws';
...
export class TabelaDivergenciaTipoComponent {
  ...
  clickedRows = new Set<DivergenciaTipoWS>();
  selection = new SelectionModel<DivergenciaTipoWS>(true, []);

  // Armazena todas as requisições que foram alteradas em tela
  private requisicoesAlteradas: DivergenciaTipoWS[] = [];

  // Armazena o backup de todas as requisições alteradas
  private requisicoesBackup: DivergenciaTipoWS[] = [];

  constructor(private _liveAnnouncer: LiveAnnouncer) {
    this.headerColumns = Colunas.colunasDivergenciaTipo;
